...
def get_features(line, pos_model, emo_dict):
	try:
		linetext = line.get_text()
		#print(linetext)
		linetext = normalize_characters(linetext.strip())
		try:
			emotions = emo_dict[linetext]
		except KeyError:
			raise KeyError(linetext)
		if emotions[2] == 'NONE':
	...
	if len(meter) == len(syll_seq):
		return zip(range(1, len(meter)+1), syll_seq, meter, rhythm, feet, caesuras, simple_pos_syll, pos_syll, syllable_position, [measure]*len(meter), [versemeasure]*len(meter), [meter]*len(meter), [emotions[0]]*len(meter))
...
if __name__ == '__main__':
	...
	emofilepath = sys.argv[3]
	emo_dict = map_lines_emotions(emofilepath)
	...
	for poem in poems:
		...
		for stanza in poem.get_stanzas():
			for line in stanza.get_line_objects():
				feature_line = get_features(line, pos_model, emo_dict)
	...
	trainfile = open('german.meter.train.6emofold.txt', 'w')
	for f_line in X_train:
		...
		trainfile.write('\n')
	trainfile.close()
	testfile = open('german.meter.test.6emofold.txt', 'w')
	for f_line in X_test:
		...
		testfile.write('\n')
	testfile.close()
	valfile = open('german.meter.dev.6emofold.txt', 'w')
	for f_line in X_val:
		for i in f_line:
			line = '\t'.join([str(m) for m in i])
			valfile.write(line)
